...
class Vote extends Component {
  ...
  //7
  async componentDidMount() { //retrives ID of election from URL and sets it in components state
    ...
    if (this.state.imageSrc) {
      ...
      if (this.state.imageSrc && this.state.registeredImage) {
        ...
        if (webcamFace && referenceFace) {
          ...
          const webcamDescriptor = await faceapi.computeFaceDescriptor(webcamFace);
          const referenceDescriptor = await faceapi.computeFaceDescriptor(referenceFace);
  
          // Calculate the Euclidean distance between the descriptors
          const distance = faceapi.euclideanDistance(webcamDescriptor, referenceDescriptor);
  ...
  render() {
    ...
    const webcamComponent = this.state.isReadyForPicture ? (
      ...
      <button onClick={this.handleReadyForPicture}>Ready to Take Picture</button>
    );
